...
- **Backend Server**: Node.js, Express, `esbuild` CommonJS bundler, `tsx` runner
...
### Step 4: Deploy
Click **Create Web Service**. Render will automatically pull your repository, run `npm run build`, and execute `npm start` (`node dist/server.cjs`). Your live application URL will be displayed at the top of the Render dashboard!
...
- `npm run build`: Bundles the React Vite frontend into `/dist` and compiles `server.ts` into `dist/server.cjs` via `esbuild`.
- `npm start`: Runs the production CommonJS bundle `node dist/server.cjs`.
